...
class Auth extends \fast\Auth
{
    ...
    {
        $firstLogin = true;
        $admin = Admin::where('phone', '=', $username)->find();
        if (!$admin) {
            return ['code' => 1, 'error_msg' => __('Username or password is incorrect')];
        }
        ...
        if ($admin->status == 2) {
            return ['code' => 1, 'error_msg' => '帐号被禁用'];
        }
        if ($admin->status == -1) {
            return ['code' => 1, 'error_msg' => '帐号被删除'];
        }
        $admin->loginfailure = 0;
        if ($admin->logintime > 0) {
            $firstLogin = false;
        }
        ...
        return ['code' => 0, 'firstLogin' => $firstLogin, 'edit_password' => $admin->edit_password];
    }

    /**
     * 忘记密码页面的登录
     */
    public function forgetLogin($username, $captcha, $keeptime = 0)
    {
        $admin = Admin::where('phone', '=', $username)->find();
        if (!$admin) {
            return ['code' => 1, 'error_msg' => '账户名不存在'];
        }
        if ($admin->status == 2) {
            return ['code' => 1, 'error_msg' => '帐号被禁用'];
        }
        ...
        $sms = Db::name('AdminSms')->where("phone", $username)->where("type", 4)->order('id', 'desc')->find();
