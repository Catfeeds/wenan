...
class Group extends Backend
{
    ...
    protected $groupdata = [];
    //无需要权限判断的方法
    protected $noNeedRight = ['roletree', 'allroletree'];
    ...
    public function allroletree()
    {
        ...
        if (($pid || $parentgroupmodel) && (!$id || $currentgroupmodel)) {
            ...
            if (!$id || !in_array($pid, Tree::instance()->getChildrenIds($id, TRUE))) {
                ...
                //所有能进到系统管理->权限管理页面的用户可以设置这3个栏目下的权限 66: 用户大厅 67: 系统管理 130: 医生站  by2018-6-11
                foreach ($ruleTreeArray as $k => $v) {
                    if (! in_array($v['id'], [66, 67, 130])) {
                        unset($ruleTreeArray[$k]);
                    }
                }
                $ruleList = Tree::instance()->getTreeList($ruleTreeArray, 'name');
                $hasChildrens = [];
                foreach ($ruleList as $k => $v) {
                    if ($v['haschild']) {
                        $hasChildrens[] = $v['id'];
