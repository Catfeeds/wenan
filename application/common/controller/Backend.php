...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (!$this->auth->match($this->noNeedLogin))
        {
            ...
            if (!$this->auth->isLogin()) {
                ...
                //未登录,访问后台首页直接跳转登录页
                if($modulename =='admin' && $actionname== 'index' && $controllername=='index'){
                    $this->redirect(url('index/login'));
                }else{
                    $this->error(__('Please login first'), url('index/login', ['url' => $url]));
                }
            } else {
                //账户在登录状态时被禁用，下一次操作后退出
                $adminInfo = Model("Admin")->get($this->auth->id);
                if ($adminInfo['status'] == 2) {
                    $this->auth->logout();
                    $this->error('帐号被禁用', 'index/login');
                } elseif ($adminInfo['status'] == -1) {
                    $this->auth->logout();
                    $this->error('帐号被删除', 'index/login');
                }
            }
            // 判断是否需要验证权限
            ...
            if (!empty($admin) && $admin->edit_password === 0 && !in_array($actionname, ['modifypassword', 'logout'])) {
                $referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
                $pos = strpos($referer, 'admin/index/forget');
                if ($pos === false) {
                    $this->redirect('/admin/general/profile/modifyPassword', [], 302);
                } else {
                    $this->redirect('/admin/general/profile/modifyPassword?from=forget', [], 302);
                }
            }
        }
    ...
    protected function buildparams($searchfields = null, $relationSearch = null)
    {
        ...
        return [$whereFunc, $sort, $order, $offset, $limit, $where];
    }
