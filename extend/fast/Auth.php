...
class Auth
{
    ...
    public function getGroups($uid)
    {
        ...
        // 转换表名
        $admin = Loader::parseName($this->config['admin'], 1);
        $auth_group = Loader::parseName($this->config['auth_group'], 1);
        // 执行查询
        $user_groups = Db::view($admin, 'id,group_id')
                ->view($auth_group, 'id,pid,name,rules', "{$admin}.group_id={$auth_group}.id", 'LEFT')
                ->where("{$admin}.id='{$uid}' and {$auth_group}.status='normal'")
                ->select();
